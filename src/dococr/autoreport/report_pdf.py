...
results section (CER before/after + reduction, model vs baselines, charts), and
conclusions. Degrades to a Markdown file if reportlab is unavailable.
"""
...
_SECTIONS = [
    ...
    ("4. Correction Quality Evaluation", "evaluation.md"),
...
def _results_table(arts: Dict[str, Any], styles):
    ...
    flow = [Paragraph("Results — post-OCR correction (test slice)", styles["H3"])]
    ev = arts.get("eval")
    rows = [["Metric", "Identity (raw OCR)", "Corrector (trained)"]]
    if ev and "model" in ev:
        m = ev["model"]
        b = ev.get("baseline_identity", {})
        for key, label in [("cer", "CER ↓"), ("wer", "WER ↓"), ("exact_match", "ExactMatch ↑"),
                           ("cer_reduction_rel", "CER reduction ↑"), ("pct_degraded", "% degraded ↓")]:
            mv = m.get(key)
            bv = b.get(key)
    ...
    t = Table(rows, hAlign="LEFT", colWidths=[140, 130, 130])
...
def generate_report(cfg: AppConfig, title: Optional[str] = None, author: Optional[str] = None,
    ...
    styles = {
        "Title": ParagraphStyle("T", parent=base["Title"], fontSize=23, leading=27),
    ...
    story: List[Any] = [Spacer(1, 5 * cm), Paragraph(title, styles["Title"]), Spacer(1, 1 * cm),
                        ...
                        Paragraph("Document-level OCR with layout & reading order + a trainable "
                                  "post-OCR correction model + an agentic pipeline.", styles["Meta"]),
    ...
    for name in ("cer_reduction", "safety", "robustness"):
        if name in built:
            story += [Image(str(built[name]), width=14 * cm, height=7.5 * cm), Spacer(1, 6)]
    story.append(PageBreak())
