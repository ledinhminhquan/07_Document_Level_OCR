...
def deskew(gray: np.ndarray, max_deg: float = 15.0) -> np.ndarray:
...
def preprocess_image(img, cfg: PreprocessConfig) -> Tuple[Image.Image, Dict[str, float]]:
    gray = to_gray(img)
    if cfg.denoise:
        gray = denoise(gray)
    if cfg.deskew:
...
__all__ = ["to_gray", "binarize", "denoise", "deskew", "quality_metrics", "preprocess_image", "otsu_threshold"]
