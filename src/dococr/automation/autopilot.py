...
def run_autopilot(cfg: AppConfig, title: str = None, author: str = None,
    ...
    _step(steps, "build_corpus", lambda: __import__(
        "dococr.data.download_dataset", fromlist=["prepare_corpus"]).prepare_corpus(cfg))
    if train:
        _step(steps, "train_corrector", lambda: __import__(
            "dococr.training.train_corrector", fromlist=["train_corrector"]).train_corrector(cfg, limit=limit))
    _step(steps, "evaluate", lambda: __import__(
        "dococr.training.evaluate", fromlist=["evaluate"]).evaluate(cfg))
    _step(steps, "benchmark", lambda: __import__(
        "dococr.analysis.latency", fromlist=["benchmark"]).benchmark(cfg, n=20, warmup=2))
    _step(steps, "error_analysis", lambda: __import__(
        "dococr.analysis.error_analysis", fromlist=["error_analysis"]).error_analysis(cfg))
    _step(steps, "robustness", lambda: __import__(
        "dococr.analysis.robustness", fromlist=["robustness_report"]).robustness_report(cfg, limit=60))

    stamp = utc_stamp()
    sub = artifacts_dir() / "submission" / f"submission-{stamp}"
    sub.mkdir(parents=True, exist_ok=True)
    report = _step(steps, "report", lambda: __import__(
        "dococr.autoreport.report_pdf", fromlist=["generate_report"]).generate_report(
        cfg, title=title, author=author, out_path=sub / "report.pdf"))
    slides = _step(steps, "slides", lambda: __import__(
        "dococr.autoreport.slides_pptx", fromlist=["generate_slides"]).generate_slides(
        cfg, title=title, author=author, out_path=sub / "slides.pptx"))
    checklist = _step(steps, "grading", lambda: __import__(
        "dococr.grading.checklist", fromlist=["build_checklist"]).build_checklist(
